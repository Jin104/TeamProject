...
int rand_x = 0; //gotoxy로 임의의 좌표로 먹이를 놓기 위한 변수
...
void gotoxy(int x, int y) //행 x, 열 y 좌표로 커서를 이동시키는 함수
{

	COORD Pos = { x - 1, y - 1 }; //COORD는 좌표값을 저장하는 구조체로 <Windows.h>를 포함(include)해서 사용한다.
								  //값을 대입할 때는 점(.)을 이용해 구조체의 멤버변수로 접근한다.
